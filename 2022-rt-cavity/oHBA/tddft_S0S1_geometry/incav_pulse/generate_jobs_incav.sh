...
do
	...
	# we now change the HOMO 2 LUMO transition --> pulse excitation of LP
	sed -i "s/field_type delta/field_type gaussian 0.0 400.0 $LP_freq/" oHBA_rgs_$i.in
 	sed -i "s/field_amp 0.000/field_amp $Amp/" oHBA_rgs_$i.in
	sed -i 's/electronic_HOMO2LUMO true/electronic_HOMO2LUMO false \nFIELD_DIRECTION 1/' oHBA_rgs_$i.in
	sed -i 's/rt_thresh 3/IN_CAVITY_EXCITE_MOLECULE true \nrt_thresh 3/' oHBA_rgs_$i.in
