...
def prepare_PT_Figb():
    pathname_outcav = "../oHBA/tddft_S0S1_geometry/outcav_HOMOLUMO/Geom_0/"
    pathname_incav = "../oHBA/tddft_S0S1_geometry/incav_HOMOLUMO/Geom_0/"
    pathname_new = "../oHBA/tddft_S0S1_geometry/incav_HOMOLUMO_coupling/Geom_0_Coupling_6e-3/"
    OD_rgs_Ang = np.array([1.008708,   1.658363,   0.000000])
    OA_rgs_Ang = np.array([2.682115,  -0.219090,   0.000000])
    OD_OA_dis = np.sum((OD_rgs_Ang - OA_rgs_Ang)**2)**0.5
    print("OD-OA separation is %.6f" %OD_OA_dis)
    H0_au = np.array([3.5866e+00,  2.1554e+00, 1.1339e-15])
    ...
    t_fs_new, R_HOD_new, R_HOA_new = get_PT_traj(pathname_new, OD_rgs_Ang, OA_rgs_Ang, H0_au)
    xs = [t_fs_out, t_fs_out, t_fs_in, t_fs_in, t_fs_new, t_fs_new]
    ys = [R_HOD_out, R_HOA_out, R_HOD_in, R_HOA_in, R_HOD_new, R_HOA_new]
    colors = [clp.navy_blue, clp.navy_blue, clp.red, clp.red,  clp.green, clp.green]
    labels = [r"H--O$_{\rm D}$ cavity off", r"H--O$_{\rm A}$ cavity off",
              r"H--O$_{\rm D}$ cavity on", r"H--O$_{\rm A}$ cavity on",
              r"H--O$_{\rm D}$ $\varepsilon=6\times 10^{-3}$ a.u.", r"H--O$_{\rm A}$ $\varepsilon=6\times 10^{-3}$ a.u."]
    return xs, ys, colors, labels

def prepare_PT_Figc():
    pathname_outcav = "../oHBA/tddft_S0S1_geometry/outcav_HOMOLUMO/Geom_10/"
    pathname_incav = "../oHBA/tddft_S0S1_geometry/incav_HOMOLUMO/Geom_10/"
    pathname_new = "../oHBA/tddft_S0S1_geometry/incav_HOMOLUMO_coupling/Geom_10_Coupling_6e-3/"
...
def prepare_PT_Figd():
    pathname_outcav = "../oHBA/tddft_S0S1_geometry/outcav_HOMOLUMO/Geom_20/"
    pathname_incav = "../oHBA/tddft_S0S1_geometry/incav_HOMOLUMO/Geom_20/"
    pathname_new = "../oHBA/tddft_S0S1_geometry/incav_HOMOLUMO_coupling/Geom_20_Coupling_6e-3/"
    OD_rgs_Ang = np.array([1.019570, 1.730158,   0.000000])
    OA_rgs_Ang = np.array([2.655089, -0.337865,   0.000000])
    OD_OA_dis = np.sum((OD_rgs_Ang - OA_rgs_Ang)**2)**0.5
    print("OD-OA separation is %.6f" %OD_OA_dis)
    H0_au = np.array([3.6417e+00,   2.3292e+00,  -6.3590e-15])
    t_fs_out, R_HOD_out, R_HOA_out = get_PT_traj(pathname_outcav, OD_rgs_Ang, OA_rgs_Ang, H0_au)
    t_fs_in, R_HOD_in, R_HOA_in = get_PT_traj(pathname_incav, OD_rgs_Ang, OA_rgs_Ang, H0_au)
    t_fs_new, R_HOD_new, R_HOA_new = get_PT_traj(pathname_new, OD_rgs_Ang, OA_rgs_Ang, H0_au)
    xs = [t_fs_out, t_fs_out, t_fs_in, t_fs_in, t_fs_new, t_fs_new]
...
def prepare_spectrum(idx):
    ...
    pathname_new = "../oHBA/tddft_S0S1_geometry/incav_LR_coupling/Geom_%d_LR_Coupling_6e-3/" %idx
    sp_out, freq_out = get_polariton_spectrum(pathname_outcav)
    sp_in, freq_in = get_polariton_spectrum(pathname_incav)
    sp_new, freq_new = get_polariton_spectrum(pathname_new)
    xs = [freq_out, freq_in, freq_new]
    ys = [sp_out, sp_in, sp_new]
    colors = [clp.navy_blue, clp.red, clp.green]
    labels = ["cavity off", "cavity on", r"$\varepsilon=6\times 10^{-3}$ a.u."]
    return xs, ys, colors, labels
...
clp.plotone(xs, ys, axins_c, colors=colors, labels=labels, showlegend=False,
            lw=1, xlabel="energy [eV]", xlim=[2.5, 4.5], ylim=[0, 23])
...
xs, ys, colors, labels = prepare_spectrum(idx=20)
...
fig.text(0.5, -0.08, r"increasing O$_{\rm D}$-O$_{\rm A}$ distance", fontsize=14, color=clp.sky_blue)
...
clp.adjust(savefile="PT_coupling.pdf")
