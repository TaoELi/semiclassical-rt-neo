...
for dir in methods:
    for dt in dt_lst:
        ...
        local_data = {}
        local_data["t"] = data[:,1] * au2fs
        local_data["mux"] = data[:,2]
        local_data["muy"] = data[:,3]
        local_data["muz"] = data[:,4]
        ...
        spy, freq = pade(t, muy)
        spz, freq = pade(t, muz)
        sptot = -freq * (np.imag(spx) + np.imag(spy) + np.imag(spz))
...
for idx, dir in enumerate(methods):
    ...
    for j, sp in enumerate(sp_lst):
        axes[idx, 1].vlines(x=0.2215 * eV2cminv, color="0.5", lw=5.0, ls="-", ymax=1.0 + j*1.1, ymin=j*1.1, alpha=0.5)
        axes[idx, 1].vlines(x=0.4569 * eV2cminv, color="0.5", lw=5.0, ls="-", ymax=0.3416 / (0.3560*2) + j*1.1, ymin=j*1.1, alpha=0.5)

    ys = [y * 1e5 + j*3 for j, y in enumerate(ys)]
    sp_lst = [sp + j*1.1 for j, sp in enumerate(sp_lst)]
    ...
    clp.plotone(freq_lst, sp_lst, axes[idx, 1], colors=colors, labels=labels, 
                xlim=[1300, 4000], 
                ylim=[-0.001, 4.57], showlegend=False,
                lw=1.0)

axes[0,1].legend(loc='upper center', #bbox_to_anchor=(1, 0.45), 
...
axes[0, 1].set_ylabel("$A_{\\rm n}(\omega)$ [arb. units]")
axes[1, 1].set_ylabel("$A_{\\rm n}(\omega)$ [arb. units]")
...
ab = AnnotationBbox(imagebox, (3100, 3.6), frameon=False)
ab.set(zorder=-1)
axes[1,1].add_artist(ab)

clp.adjust(tight_layout=False, savefile="figs1_mun_linear_response_imag.pdf")
