...
dirs = [
        "../run_pbe_6-31gd/eq_rt_pulse_1.2e-2", "../run_pbe_6-31gd/eq_neo_pulse_1.2e-2", "../run_pbe_6-31gd/eq_rt_pulse_1.2e-2_D", "../run_pbe_6-31gd/eq_neo_pulse_1.2e-2_D",
        ]
...
ax = clp.initialize(1, 1, width=3.25, height=3.25*0.618, LaTeX=False, fontsize=10, labelsize=11)
axes = [ax]
...
clp.plotone(xs[0:4], ys[0:4], axes[0], colors=colors, labels=labels, linestyles=linestyles,
            ...
            ylim=[0.71, 2.0], ylabel=r"H-H or D-D distance [Å]",
            xlabel="time [fs]",
            lw=1.5)
...
clp.plotone(xs[4:8], ys[4:8], axes[1], colors=colors, labels=labels, linestyles=linestyles,
            ...
            ylabel="H-H or D-D interatomic distance [$\mathrm{\AA}$]",
            ylim=[0.71, 2.0],
            lw=1.5)
...
lgd = axes[0].legend(loc='center left', bbox_to_anchor=(1, 0.5), fancybox=True,
            facecolor='inherit', edgecolor="inherit", fontsize=8)

clp.adjust(tight_layout=False, savefile="figs10_bond_length_dynamics_pulse_intensity_sc_response.pdf", includelegend=lgd)
