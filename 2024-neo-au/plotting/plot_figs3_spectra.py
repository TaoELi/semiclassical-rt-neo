...
for filename in methods:
    # from this filename to get x, y, z three separate calculations
    filename_x = filename.replace("all", "all_x")
    filename_y = filename.replace("all", "all_y")
    filename_z = filename.replace("all", "all_z")
    # open the file for proton dipole moment
    print("working under %s" %filename_z)
    ...
    sptot = freq * np.abs(spx)

    data = np.loadtxt(filename_y)
    dt = data[1,0] - data[0,0]
    nskip = int(4.0 / float(dt))
    if nskip < 1:
        nskip = 1
    ...
    spz, freq = pade(t, muz)
    #sptot += freq * (np.abs(spx) + np.abs(spy) + np.abs(spz))
    sptot += freq * np.abs(spy)

    data = np.loadtxt(filename_z)
    dt = data[1, 0] - data[0, 0]
    nskip = int(4.0 / float(dt))
    if nskip < 1:
        nskip = 1
...
clp.adjust(tight_layout=False, savefile="figs3_mu_linear_response_sep.pdf")
