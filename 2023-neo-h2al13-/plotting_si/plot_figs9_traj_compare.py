...
methods = ["%s" %x for x in ["../run_pbe_6-31gd/eq_neo_pulse_2e-2",
                             "../run_pbe_6-31gd/eq_neo_pulse_2centers",
                             "../run_pbe_6-31gd/eq_neo_pulse_3centers"]]
dt = 0.1
...
colors = [clp.red, clp.sky_blue, clp.dark_green]
linestyles = ["-", "--", "-."]
labels = ["RT-NEO-TDDFT TPB H$_2$", "RT-NEO-TDDFT FPB/2c H$_2$", "RT-NEO-TDDFT FPB/3c H$_2$"]
...
clp.plotone(xs, ys, ax, colors=colors, labels=labels, linestyles=linestyles,
            showlegend=True, xlim=[0, 15], xlabel="time [fs]", legendFontSize=9,
            ylabel=r"H-H interatomic distance [Å]", ylim=[0.71, 2.4],
            lw=1.5)


clp.adjust(tight_layout=False, savefile="figs9_bond_length_dynamics_compare.pdf")
